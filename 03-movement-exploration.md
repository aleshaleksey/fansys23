...
There are no general hazards in the shallows of the Spirit Realm. However, upon leaving, the traveller must make an INT save (Threshold=16) or be disoriented by the passage between realms. An entity that fails this save cannot succeed "navigate" rolls for the next 24 hours. If the traveller rolls snake eyes, they forget the way to that spirit realm. If they entered via a secret path, they forget that path and must find it anew. If they used a different modality, they cannot use it again unless (to enter that realm) they are taught it again by the ruler of that realm.
